...
export default function ProductDetail() {
  const params = useParams({ strict: false }) as { storeId?: string; productId?: string };
  const navigate = useNavigate();
  const storeId = params.storeId ?? undefined;
  const productId = params.productId ?? undefined;

  const { data: product, isLoading, isError } = useGetProduct(storeId, productId);
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="outline" className="text-xs capitalize">{productTypeLabel}</Badge>
              {statusLabel !== 'active' && (
                <Badge variant="secondary" className="text-xs">Unavailable</Badge>
              )}
            </div>
            <h1 className="text-3xl font-bold text-foreground">{typedProduct.title}</h1>
            {typedProduct.category && (
              <p className="text-sm text-muted-foreground mt-1">{typedProduct.category}</p>
            )}
          </div>
          ...
          {typedProduct.description && (
            <p className="text-muted-foreground leading-relaxed">{typedProduct.description}</p>
          )}
              ...
              >
                ...
                <SelectContent>
                  {variants.map((variant, idx) => {
                    ...
                    return (
                      <SelectItem key={idx} value={String(idx)} disabled={vStock <= 0}>
          ...
          {!inStock && (
            <p className="text-sm text-destructive bg-destructive/10 rounded-lg px-3 py-2">
              This option is currently out of stock. Please select a different variant or check back later.
            </p>
          )}
          ...
          <Button
            ...
            disabled={!inStock || addToCart.isPending || (hasVariants && selectedVariantIndex === null)}
          >
            {addToCart.isPending ? (
              ...
              <>
                ...
                {!inStock ? 'Out of Stock' : hasVariants && selectedVariantIndex === null ? 'Select an Option' : 'Add to Cart'}
              </>
            )}
          </Button>
        </div>
