...
export default function AuctionCountdown({ endTime, className = '', compact = false }: AuctionCountdownProps) {
  ...
  if (timeLeft.ended) {
    return (
      <span className={`inline-flex items-center gap-1 text-muted-foreground font-sans text-sm ${className}`}>
  ...
  if (compact) {
    ...
    return (
      <span className={`inline-flex items-center gap-1 font-mono text-sm text-amber-600 dark:text-amber-400 ${className}`}>
        <Clock className="w-3.5 h-3.5" />
        {parts.join(' ')}
      </span>
    );
      ...
      <div className="flex gap-2">
        {timeLeft.days > 0 && (
          <div className="text-center">
            <div className="font-mono text-lg font-bold text-foreground leading-none">{timeLeft.days}</div>
