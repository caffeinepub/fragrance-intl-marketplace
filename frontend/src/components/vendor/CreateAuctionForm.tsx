...
export default function CreateAuctionForm({ storeId, _vendorId, onCreated }: CreateAuctionFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="text-sm font-medium text-foreground mb-1 block">Product ID</label>
        <Input value={productId} onChange={(e) => setProductId(e.target.value)} placeholder="product-id" />
      </div>
      <div>
        <label className="text-sm font-medium text-foreground mb-1 block">Starting Price ($)</label>
        <Input
          type="number"
          step="0.01"
          min="0.01"
      ...
      <Button type="submit" disabled={createAuction.isPending} className="w-full">
