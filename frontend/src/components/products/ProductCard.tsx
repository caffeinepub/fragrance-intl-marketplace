...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const selectedVariant = selectedVariantIndex !== undefined ? variants[selectedVariantIndex] : undefined;
  const basePrice = typeof product.price === 'number' ? product.price : Number(product.price);
  const baseStock = typeof product.stock === 'number' ? product.stock : Number(product.stock);
  ...
  const getImageUrl = (): string | null => {
    ...
    try {
      const principalStr = typeof product.image === 'object' && 'toString' in product.image
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex-1">
          ...
          {hasVariants && (
            <div className="mb-3">
              <select
                value={selectedVariantIndex !== undefined ? String(selectedVariantIndex) : ''}
                onChange={(e) => {
              ...
              >
                {variants.map((v, i) => {
                  ...
                  return (
                    <option key={i} value={String(i)} disabled={vStock <= 0}>
