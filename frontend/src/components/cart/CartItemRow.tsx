...
export default function CartItemRow({ item, product }: CartItemRowProps) {
  ...
  const variant =
    product && item.variantIndex !== undefined
      ...
      : null;
  const effectivePrice = variant ? basePrice + variant.priceAdjustment : basePrice;
    ...
    <div className="flex items-center gap-4 py-4 border-b border-border last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-sm text-foreground truncate">
          {product?.title ?? <span className="font-mono text-xs">{item.productId.slice(0, 20)}…</span>}
        </p>
        {variantLabel && (
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <Button
          ...
          onClick={() =>
            removeFromCart.mutate({ productId: item.productId, variantIndex: item.variantIndex })
          }
