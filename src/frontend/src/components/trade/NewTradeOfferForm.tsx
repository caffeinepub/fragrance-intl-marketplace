        ...
        offeredItems: [
          {
            ...
            quantity: BigInt(Number.parseInt(offeredQty) || 1),
          },
        ],
        requestedItems: [
          {
            productId: requestedProductId,
            quantity: BigInt(Number.parseInt(requestedQty) || 1),
          },
        ],
        cashAdjustment:
          Math.round(Number.parseFloat(cashAdjustment) * 100) || 0,
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label
            htmlFor="trade-offered-product"
            className="text-sm font-medium text-foreground mb-1 block"
          >
            Offered Product ID
          </label>
          <Input
            id="trade-offered-product"
        ...
        <div>
          <label
            htmlFor="trade-offered-qty"
            className="text-sm font-medium text-foreground mb-1 block"
          >
            Quantity
          </label>
          <Input
            id="trade-offered-qty"
            type="number"
            min={1}
            value={offeredQty}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label
            htmlFor="trade-requested-product"
            className="text-sm font-medium text-foreground mb-1 block"
          >
            Requested Product ID
          </label>
          <Input
            id="trade-requested-product"
        ...
        <div>
          <label
            htmlFor="trade-requested-qty"
            className="text-sm font-medium text-foreground mb-1 block"
          >
            Quantity
          </label>
          <Input
            id="trade-requested-qty"
            type="number"
            min={1}
            value={requestedQty}
      ...
      <div>
        <label
          htmlFor="trade-cash-adjustment"
          className="text-sm font-medium text-foreground mb-1 block"
        >
          Cash Adjustment ($)
        </label>
        <Input
          id="trade-cash-adjustment"
          type="number"
          step="0.01"
