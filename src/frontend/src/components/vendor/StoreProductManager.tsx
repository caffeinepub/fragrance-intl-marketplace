...
function buildBackendProduct(
  ...
  return {
    ...
    } as unknown as BackendProduct["productType"],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const variants: ProductVariant[] = variantRows.map((row) => ({
      ...
      priceAdjustment: Math.round(
        Number.parseFloat(row.priceAdjustment || "0") * 100,
      ),
      stockAdjustment: Number.parseInt(row.stockAdjustment || "0", 10) || 0,
    }));

    const productId = product?.id ?? `product_${Date.now()}`;
    const backendProduct = buildBackendProduct(
      productId,
      vendorId,
      form,
      variants,
...
export default function StoreProductManager({
  storeId,
  vendorId,
}: StoreProductManagerProps) {
  const {
    data: rawProducts,
    isLoading,
    isError,
  } = useListStoreProducts(storeId);
    ...
    <div className="space-y-4">
            ...
            <TableBody>
              {products.map((product) => (
                <TableRow key={product.id}>
                  ...
                  <TableCell className="font-sans text-sm text-right">
                    {product.stock}
                  </TableCell>
      ...
      >
        <AlertDialogContent className="bg-card border-border">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="font-sans border-border">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
