...
function AuctionStatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className={`text-xs ${variants[status] ?? "bg-muted text-muted-foreground"}`}
    >
      {status}
    </Badge>
  );
        ...
        <div className="grid grid-cols-2 gap-4 text-sm font-sans">
          <div>
            ...
            <p className="text-foreground font-medium">
              ${(typedAuction.startingPrice / 100).toFixed(2)}
            </p>
          </div>
          {typedAuction.currentBid != null && (
            <div>
              ...
              <p className="text-gold font-medium text-lg">
                ${(typedAuction.currentBid / 100).toFixed(2)}
              </p>
            </div>
          )}
          {typedAuction.reservePrice != null && (
            <div>
              ...
              <p className="text-foreground font-medium">
                ${(typedAuction.reservePrice / 100).toFixed(2)}
              </p>
            </div>
          )}
          <div>
            ...
            <p className="text-foreground font-medium">
              {typedAuction.status === "active" ? countdown : "Auction ended"}
            </p>
          </div>
          <div>
            <span className="text-muted-foreground flex items-center gap-1">
            ...
            <p className="text-foreground font-medium">
              {typedAuction.bids.length} bid
              {typedAuction.bids.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
          ...
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded p-3">
            <span className="font-sans text-sm text-emerald-600">
              ...
              <span className="font-mono">
                {String(typedAuction.currentBidder).slice(0, 12)}…
              </span>
            </span>
          </div>
        )}
